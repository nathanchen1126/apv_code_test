...
# -------------------- 空间参数 --------------------
IMPACT_RADIUS   = 3000
BG_INNER_RADIUS = 6000
BG_OUTER_RADIUS = 10000
...
def compute_apv_efficiency(ndvi_path: Path):
    # ★ MOD: 如果已存在效率表，直接读取
    if APV_RATE_XLSX.exists():
        return pd.read_excel(APV_RATE_XLSX)
...
def derive_scenarios(apv_rates: pd.DataFrame):
    df = apv_rates.dropna(subset=["rate_ndvi"]).copy()
    df = df[df["rate_ndvi"] > 0]   # ★ MOD: 几何均值必须 >0

    g = df.groupby("climate_zone")["rate_ndvi"]

    rate_gmean = g.apply(lambda x: np.exp(np.mean(np.log(x))))

    return pd.DataFrame({
        "climate_zone": rate_gmean.index,
        "rate_top": g.quantile(0.9).values,
        "rate_med": rate_gmean.values,   # ★ MOD: 几何均值替代中位数
